...
![](https://raw.githubusercontent.com/datasciencelabs/2022/refs/heads/master/dataviz/img/steel-is-back.png){fig-align="center" height=500}

## Know when to include 0


![](https://raw.githubusercontent.com/datasciencelabs/2022/refs/heads/master/dataviz/img/steel-is-back-community-note.png){fig-align="center"}

## Know when to include 0

Here is the appropriate plot:

```{r barplot-from-zero-4}
data.frame(Candidate = factor(c("2024 Total Steel Production (Mt)", "2025 Total Steel Production (Mt)"), levels = c("2024 Total Steel Production (Mt)", "2025 Total Steel Production (Mt)")),
           Percent = c(80.9, 81.8)) |>
  ggplot(aes(Candidate, Percent, fill = Candidate)) +
  geom_bar(stat = "identity", width = 0.65, show.legend = FALSE)  +
  ggtitle("Total Steel Production (Mt) 2024 v. 2025")
```
...
## Choosing Effective Axis Ranges

* Most statistical software default to using the data range to define axies range.

* This is usually the best choice.

* However, outliers can result in wasting area in the plot that ends up hiding important patterns

## Choosing Effective Axis Ranges

```{r effective-axis, echo=FALSE}
n <- 500 
x <- rnorm(n, 8, 4)
y <- 2*x + rnorm(n, 8, 4)
y[which.max(x)] <- y[which.max(x)]*25
plot(x, y)
```

## Choosing Effective Axis Ranges

```{r effective-axis-2, echo=FALSE}
plot(x, y, ylim = c(1, max(y[-which.max(y)])))
```

## Choosing Effective Axis Ranges

![](https://raw.githubusercontent.com/datasciencelabs/2022/refs/heads/master/dataviz/img/x-axis-too-big.png){fig-align="center" height=500}

## Choosing Effective Axis Ranges

![](https://raw.githubusercontent.com/datasciencelabs/2022/refs/heads/master/dataviz/img/x-axis-too-big-2.png){fig-align="center" height=500}
...
## Stop putting UMAP plots in your papers
...
```{r iris-example, echo=FALSE}
plot(iris[,-5])
```

## What Does UMAP Actually Do?

```{r mnist-example, echo=FALSE}
...
## This is UMAP of negative controls

![](https://raw.githubusercontent.com/datasciencelabs/2022/refs/heads/master/dataviz/img/umap.jpeg){fig-align="center" height=500}
